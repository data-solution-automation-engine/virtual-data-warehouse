...
IF EXISTS (SELECT * FROM sys.objects WHERE type ='P' AND name ='usp_MD_DV_METADATA_VERSIONED')
DROP PROCEDURE usp_MD_DV_METADATA_VERSIONED
GO

CREATE PROCEDURE [dbo].[usp_MD_DV_METADATA_VERSIONED] @VERSION_ID  int
AS
BEGIN
	SET NOCOUNT ON;	
...
FROM 
( 
	SELECT DISTINCT COLUMN_NAME FROM MD_VERSION_ATTRIBUTE
...
INNER JOIN MD_VERSION_ATTRIBUTE vat 
	ON lnk.LINK_TABLE_NAME = vat.TABLE_NAME 
   AND hub.HUB_TABLE_NAME = REPLACE('HUB_'+vat.COLUMN_NAME,'_HSH','')
WHERE
	VERSION_ID = @VERSION_ID
	AND vat.DRIVING_KEY_INDICATOR='Y'
...
PRINT ''
PRINT 'Processing Multi-Active Key Settings (MD_STG_SAT_ATT_XREF)';
...
(
	SELECT
		TABLE_NAME AS SATELLITE_TABLE_NAME,
		COLUMN_NAME AS ATTRIBUTE_NAME
	FROM
		MD_VERSION_ATTRIBUTE
	WHERE 
		MULTI_ACTIVE_INDICATOR='Y'
	AND VERSION_ID=@VERSION_ID
) sub
